                ...
                <div className="border-t border-slate-100 pt-3 space-y-3">
                  <div className="space-y-1.5">
                    ...
                    <select value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)}
                      ...
                      {products.map(p => <option key={p.id} value={p.id}>{p.name} (Stok: {p.totalStock || 0})</option>)}
                    </select>
                  </div>
                  <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setAddItemOpen(false)} className="flex-1 py-2.5 bg-slate-100 text-slate-600 font-bold rounded-xl active:scale-95">Batal</button>
