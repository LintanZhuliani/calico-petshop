...
export default function ProductsPage() {
  ...
  const handleConfirmCheckout = async (payload) => {
    try {
      ...
      const toastDetail = payload.paymentMethod === 'Tunai'
        ? 'Kembalian: ' + formatRupiah(payload.change || 0)
        : 'via ' + (payload.paymentMethod || 'Tunai');
      showToast('Transaksi berhasil! ' + toastDetail);
      
      // Redirect to riwayat to auto open receipt
      setTimeout(() => {
        navigate('/riwayat', { 
          state: { 
            autoOpenLatest: true 
